...
public static class CelestialLightingFeatures
{
    ...
    // Lets another mod's indoor brightening reach the screen (issue #80). §7b forces every enclosed
    // roofed cell's rendered sky-cover toward full occlusion from geometry alone; a mod that
    // redistributes sky glow into roofed cells therefore produced a real, gameplay-visible,
    // mouseover-reportable number that rendered as flat black. With this on,
    // IndoorOcclusionMath.CapOcclusion additionally honours whatever sky-sourced glow actually reached
    // the cell as a second floor alongside MinIndoorBrightness.
    //
    ...
    // instead of the mod, so any mod that brightens interiors is honoured without being named — that
    // per-mod pattern was the common root of this subsystem's compatibility bugs. See
    // IndoorGlowPassthrough and IndoorOcclusionMath.IndoorSkyGlowFraction.
    //
    // When off, or on an unmodded install, IndoorGlowPassthrough.SkyFractionAt returns 0 for every
    // cell, which is the exact identity CapOcclusion already had before this existed — the faithful
