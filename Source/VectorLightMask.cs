...
public static class VectorLightMask
{
        ...
        Map map, GlowGridPerLight.Reader reader, CellRect rect, int cells, bool lifting)
    {
        ...
        if (OverlappingLights(rect, box, reader) < 2)
        {
        ...
        // and a bucket is only the right answer for a section-aligned query, so the alignment is
        // CHECKED rather than assumed: Apply's caller builds rect from Section.botLeft so it is
        // always aligned, but a future caller that was not would otherwise read the bucket of
        // whichever section its corner landed in and lose lights with no exception and no probe
        // moving.
        //
        // THE BUCKET IS KEYED ON THE SECTION AND TESTED AGAINST THE BOX. The index files each
        // light under every section its reach touches, margin included, and the candidates' box
        // lies inside this section's margin, so the section's bucket holds every light that can
        // reach a candidate. Testing each one against the box rather than the section is what
        // drops the lights whose square reaches the section but misses every candidate.
        //
                ...
                out int bucketStart, out int bucketEnd))
        {
            ...
            for (int b = bucketStart; b < bucketEnd; b++)
            {
                ...
                // box -- and under the gate the box is smaller still. The bucket removes the lights
                // that were never close; this removes the few that are close and still miss.
    ...
    //
    // `rect` is the section, which keys the index; `box` is what a light has to reach to count,
    // and under the gate it is smaller than the section. See the fold loop for why both are needed.
    private static int OverlappingLights(CellRect rect, CellRect box, GlowGridPerLight.Reader reader)
    {
        int found = 0;
