...
// Patch_TwilightColor.
//
// Same injection point as Patch_TwilightColor: a stage of Patch_SkyTargetComposite's postfix on
// WeatherWorker.CurSkyTarget. The two blend different, non-overlapping things (twilight warms the
// sky at dusk-glow; this tints it only at deep night and only during an auroral event), so they
// stack cleanly wherever the composite sequences them. The night-visibility ramp, fade, and flare shimmer colour are all pure functions in
...
public static class Patch_AuroraTint
{
    internal static void Apply(Map map, ref SkyInputs inputs, ref SkyTarget target)
    {
        ...
        // target.glow, for the same reason Patch_TwilightColor does: the aurora's night-only
        // visibility should track true sun position, not displayed brightness. That matters
        // concretely here because §7 rewrites target.glow below the horizon with its night floor —
        ...
        target.colors.sky = Color.Lerp(target.colors.sky, tintColor, skyStrength);
        target.colors.overlay = Color.Lerp(target.colors.overlay, tintColor, overlayStrength);
    }
}
