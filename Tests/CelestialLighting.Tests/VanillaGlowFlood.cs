...
// decompiled source, sharing no code whatsoever with VectorLightMaxMath, which is the thing it
// judges.
//
// WHY THIS FILE EXISTS AT ALL, given how much of it looks like duplication. VectorLightMaxMath's
// whole claim is a DIFFERENCE — "what vanilla stored here" against "what vanilla would have stored
// had the light gone straight" — and a test that computes both halves with the code under test is
// asserting `x - x == 0`, which passes for a formula that is wrong in the same way on both sides.
// That exact test existed in an earlier attempt at this and passed while the term it guarded lifted
// every room in the game by a fifth. So the free-space claim is checked against an actual flood over
// an actual lattice: this file runs the Dijkstra, and the assertion is that a closed form nowhere
// near it lands on the same byte.
//
